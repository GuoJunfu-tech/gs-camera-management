...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        #     FovX = focal2fov(focal_length_x, width)
        # else:
        #     print(f"camera model: {intr.model}")
        #     assert False, "Colmap camera model not handled: only undistorted datasets (PINHOLE or SIMPLE_PINHOLE cameras) supported!"

        image_path = os.path.join(images_folder, os.path.basename(extr.name))
        image_name = os.path.basename(image_path).split(".")[0]
        if not os.path.exists(images_folder):
            print(f"{images_folder} not exists")
            exit()
...
def readCamerasFromArticulatedTransforms(
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, (key, raw_matrix) in enumerate(contents.items()):
            ...
            R = np.transpose(
                w2c[:3, :3]
            ).T  # R is stored transposed due to 'glm' in CUDA code
            T = w2c[:3, 3]

            image_path = os.path.join(path, cam_name)
            image_name = Path(cam_name).stem
            image = Image.open(image_path)
            image.resize((800, 800), Image.BICUBIC)
            im_data = np.array(image.convert("RGBA"))
            bg = np.array([1, 1, 1]) if white_background else np.array([0, 0, 0])
            norm_data = im_data / 255.0
            mask = norm_data[..., 3:4]
            ...
            # fovx = focal2fov(K[0][0], 800)
            # fovy = focal2fov(K[1][1], 800)
            # fovy = focal2fov(fov2focal(fovx, image.size[0]), image.size[1])
            ...
            # FovX = fovx
            # FovY = fovy  # FIXME: strange operation
...
sceneLoadTypeCallbacks = {
    ...
    "articulated": readArticulatedSyntheticInfo,
}
